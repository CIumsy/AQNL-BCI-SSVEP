...
This is why main.py -- which is single-process and correct on Windows --
...
Usage:
    python3 run_uno_q.py            # calibration + live detection
...
def main():
    ...
    proc.start()
    print(f"[run_uno_q] display process started (pid {proc.pid}), "
    ...
    try:
        ...
        print("\n[run_uno_q] interrupted.")
    finally:
        display.stop()
        proc.join(timeout=5)
        if proc.is_alive():
            proc.terminate()
        print("[run_uno_q] done.")
